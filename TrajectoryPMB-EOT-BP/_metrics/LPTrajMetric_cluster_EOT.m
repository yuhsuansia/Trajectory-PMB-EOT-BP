function [dxy, loc_cost, miss_cost, fa_cost, switch_cost]=LPTrajMetric_cluster_EOT(X, Y, c, p, gamma)
...
for i=1:length(Clusters)
    ...
    elseif(length(Cluster_i)==2)
        ...
        X_i.eState=X.eState(:,:,t_min:t_max,list_x);
        Y_i.eState=Y.eState(:,:,t_min:t_max,list_y);
                ...
                for yind = 1:ny+1
                    if (yind <= ny) && (t >= Y.tVec(yind)) && ...
                        ...
                        locCostMat(xind,yind,t) = computeLocCostPerTime( ...
                            X.xState(:,t,xind), Y.xState(:,t,yind), X.eState(:,:,t,xind), Y.eState(:,:,t,yind), c, p);
...
function d = computeLocCostPerTime(x, y, xe, ye, c, p)
if all(~isnan(x)) && all(~isnan(y))
    % neither x nor y has hole
    d = min(GaussianWassersteinDistance(x,xe,y,ye),c^p);
