function [newIndexes,measurements,likelihood1] = getPromisingNewMeasurementAllTrajectory(multiBernoulli,measurements,step,indexAliveParticle,parameters)
...
for target = 1:numTargets
    likelihood1{target} = zeros(numParticlesBernoulli(target),numMeasurements);
    indexAliveParticleTmp = indexAliveParticle{target};
    lengthIndex = length(indexAliveParticleTmp);
    tmpMean = reshape(multiBernoulli(target).particlesKinematic(1:2,step,indexAliveParticleTmp),2,lengthIndex);
    tmpCov = reshape(multiBernoulli(target).particlesExtent(:,:,step,indexAliveParticleTmp),2,2,lengthIndex) + repmat(measurementsCovariance,[1,1,lengthIndex]);
    for measurement = numMeasurements:-1:1
        likelihood1{target}(indexAliveParticleTmp,measurement) = constantFactor * exp(getLogWeightsFast(measurements(:,measurement),tmpMean,tmpCov));
